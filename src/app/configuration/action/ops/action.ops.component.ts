...
@Component({selector: 'app-action-ops', templateUrl: './action.ops.component.html', styles: [], standalone: false})
export class ActionOpsComponent extends OpsAbstract implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.crudForm = this.fb.group({
            actionCode: this.appCtrlService.generateFormControl(CONTROL_DESCRIPTION.input, this.object?.actionCode),
            actionId: this.appCtrlService.generateFormControl(CONTROL_DESCRIPTION.numeric, this.object?.actionId),
            partial: this.appCtrlService.generateFormControl(CONTROL_DESCRIPTION.switch, this.object?.partial),
            permitExcess: this.appCtrlService.generateFormControl(CONTROL_DESCRIPTION.switch, this.object?.permitExcess),
        });
    }
